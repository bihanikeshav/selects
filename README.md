...
- **Photos and video** — sampled-frame scoring, dead-footage detection, highlight segments.
- **Fast to cull** — keyboard-first review, side-by-side compare, and it learns your taste.
...
| Keyboard culling | Arrow-key review, undo, 100% zoom, synced-zoom compare view |
...
|---|:---:|---|
| Windows (x64) | yes | planned — NVIDIA / CUDA |
| macOS (Apple Silicon) | yes | planned — Metal (MPS) + CoreML |
| macOS (Intel) | yes | — |
| Linux (x64) | yes | planned — NVIDIA / CUDA |
| AMD / Intel GPUs | yes (as CPU) | later — ONNX Runtime DirectML / ROCm |
...
**Data flow:** files are walked and hashed, then classical signals (blur/exposure/faces), then ML
embeddings and tags, then face/location/theme clustering, then burst "moments", then day/place
"stories" and aesthetic curation. Each stage is independently re-runnable and writes back to the
same SQLite DB.
...
**Android apps** (two distinct products)
