...
``selects.classical.faces.detect_faces`` (the default ``FaceAnalysis``
...
Also provides:
  ...
  * a bounded burst-pick penalty used by ``selects.ml.curation`` so that
...
def run_face_attribute_stage(
    ...
    pipeline stages in selects.pipeline. Returns photos processed.
    """
    from selects.db import init_db, session_scope
    from selects.db.models import FaceEmbedding, Photo
