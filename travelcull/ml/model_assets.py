"""First-class model-weights management for travelcull.

Every ML stage in travelcull silently downloads several GB of weights on first
...
def models_dir() -> Path:
    ...
    Overridable via ``TRAVELCULL_MODELS_DIR`` (used by tests). Defaults to
    ``~/.cache/travelcull/models``.
    """
    env = os.environ.get("TRAVELCULL_MODELS_DIR")
    if env:
        return Path(env)
    return Path.home() / ".cache" / "travelcull" / "models"
