...
persisted to ``<library>/.travelcull/taste.npz`` together with metadata
(n_samples, holdout AUC, trained_at).

The taste score is blended into the curation ranking (see
:mod:`travelcull.ml.curation`) as::
...
def state_dir_from_session(s: OrmSession) -> Optional[Path]:
    """Derive <library>/.travelcull from the session's SQLite file path."""
    try:
        bind = s.get_bind()
...
def _labeled_query(s: OrmSession):
    from travelcull.db.models import Embedding, Swipe
