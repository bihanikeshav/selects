...
* scores every sampled frame with the existing classical metrics
  (:func:`travelcull.classical.blur.laplacian_variance`,
  :func:`travelcull.classical.exposure.exposure_score`),
...
def _embed_best_frame(img: np.ndarray) -> Optional[bytes]:
    ...
    try:
        ...
        from travelcull.ml.embed import encode_image_batch
...
def _save_frame_strip(cfg: FolderConfig, sha256: str, arrays: list[np.ndarray]) -> None:
    from travelcull.indexer.preview import _resize_and_save
